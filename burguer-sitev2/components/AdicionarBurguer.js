...
function AdicionarBurguer(props){
  const [addPreço, setAddPreço] = useState(0)
  const listaDeAdicionais = [7,6,6,2,0,5,7,6]
  const listaDeNomes = ['Carne', 'Frango', 'Calabresa', 'Creme cheddar', 'Molho', 'Batatas', 'Coração', 'Bacon']
  ...
  function onSubmitHandler(event){
    ...
    dispatch(storeSacolaActions.addSacola({
      id: Math.random(),
      nome: props.dadosAPI2[0].burguer[Number(props.menuId)-1].nome,
      ...
      preço: Number.parseFloat(numero*(Number(props.dadosAPI2[0].burguer[Number(props.menuId)-1].preço) + addPreço))
        ...
        <form onSubmit={onSubmitHandler}>
          <div className=" text-start">
           <div className='relative h-80 aplicar-css6 grid place-items-center rounded-2xl -m-3'>
             ...
             <div >
               <Image src={props.imagens.Burguer[Number(props.menuId)-1]}/>
             </div>
           ...
           <div className='aplicar-css4 h-28 mt-3'>
             <div>
               <h2 className=' text-xl mt-2 mb-2'><strong>{props.dadosAPI2[0].burguer[Number(props.menuId)-1].nome}</strong></h2>
               <p className=' text-sm'>{props.dadosAPI2[0].burguer[Number(props.menuId)-1].descri}</p>
             </div>
             <div className='flex justify-end mr-3 mt-2 text-xl'><strong>R${props.dadosAPI2[0].burguer[Number(props.menuId)-1].preço}</strong></div>
             </div>
           <div className='h-50'>
             ...
             <div className='flex overflow-x-scroll pb-10 hide-scroll-bar'>
               ...
               <div className="flex justify-center w-s bg-white">
                   <div className="aplicar-css5 w-28 h-40  rounded-t-2xl">
                           ...
                           <div className='flex justify-center'>Gado</div>
                       </div>
                   ...
                   <div className="aplicar-css5 w-28 h-40  rounded-t-2xl">
                           ...
                           <div className='flex justify-center'>Frango</div>
                       </div>
                   ...
                   <div className="aplicar-css5 w-28 h-40  rounded-t-2xl">
                           ...
                           <div className="grid grid-cols-2 rounded-b-2xl shadow">
                               <p className="ml-3 flex items-center text-sm "><strong>R$7,00</strong></p>
                               <div className="grid items-end justify-items-end">
                               <BotaoAdicionais onClickAddHandler={onClickAddHandler} lista= {listaAdicionados} index={'7'}/>
                               </div>
                           </div>
                   ...
                   <div className="aplicar-css5 w-28 h-40  rounded-t-2xl">
                           ...
                           <div className="grid grid-cols-2 rounded-b-2xl shadow">
                               <p className="ml-3 flex items-center text-sm "><strong>R$6,00</strong></p>
                               <div className="grid items-end justify-items-end">
                               <BotaoAdicionais onClickAddHandler={onClickAddHandler} lista= {listaAdicionados} index={'8'}/>
                               </div>
                           </div>
           ...
           <div>
             ...
             <div className='grid grid-cols-2 m-2'>
              {props.dadosAPI2[0].burguer[Number(props.menuId)-1].ingred.map((item, index) =>
           ...
           <footer className='border h-28 sticky bottom-0 grid grid-cols-2 bg-white'>
             ...
             <div className='grid place-items-center'>
               <button type='submit' className=' bg-amareloP rounded-2xl p-1 text-center shadow text-gray-900'><p className={cherry.className}>Adicionar ao carrinho <br></br> R${Number.parseFloat(numero*(Number(props.dadosAPI2[0].burguer[Number(props.menuId)-1].preço) + addPreço)).toFixed(2)}</p></button>
               </div>
             </footer>
                </div>
        </form>
...
export default AdicionarBurguer